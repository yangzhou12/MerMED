...
# --------------------------------------------------------
"""Finetune the MerMED-FM ViT backbone on a single downstream classification dataset.

Loads the MerMED checkpoint via ``--finetune`` (handles the ``student``/``teacher``
keys of the pretraining checkpoint), attaches a fresh classification head, and trains
with layer-wise LR decay. Supports few-shot training via ``--train_size`` and
evaluation-only via ``--eval``. See ``scripts/finetune_mermed.sh`` for batch runs.
"""
...
def get_args_parser():
    ...
    parser.add_argument('--data_path', default='', type=str,
                        help='dataset root directory (image paths in the label CSV are resolved against it)')
    parser.add_argument('--label_path', default='', type=str,
                        help='path to the label CSV (columns: image_id, image_path, label, split)')
...
def main(args):
    ...
    if args.finetune and not args.eval:
        ...
        if 'model' in checkpoint.keys():
            ...
            # Create a mapping of prefix replacements
            prefix_mappings = {
            ...
            # if ".ls1.gamma" in key:
            #     new_key = key.replace(".ls1.gamma", ".gamma_1")
            # if ".ls2.gamma" in key:
            #     new_key = key.replace(".ls2.gamma", ".gamma_2")

            new_checkpoint_model = {}
            for old_key in checkpoint_model.keys():
                new_key = old_key
                # Try each prefix replacement
        ...
        # Remove classification head parameters
        for k in ['head.weight', 'head.bias', 'head.fc.weight', 'head.fc.bias']:
            if k in checkpoint_model:
                print(f"Removing key {k} from pretrained checkpoint")
                del checkpoint_model[k]

        # Remove normalization layers if present
        ...
        # manually initialize fc layer only if model has a head
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_stats, val_auc_roc, val_auc_pr, val_F1 = evaluate(data_loader_val, model, device, output_dir,
        ...
        if epoch==(args.epochs-1):
            test_stats, auc_roc, auc_pr, F1 = evaluate(data_loader_test, model, device, output_dir,
