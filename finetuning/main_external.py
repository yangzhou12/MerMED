...
# --------------------------------------------------------
"""Evaluate the MerMED-FM ViT backbone on an external (held-out) test set.

Trains on one dataset and evaluates on a separate external dataset (no shared splits),
reusing the standard finetuning engine. See ``--train_*`` and ``--test_*`` arguments.
"""
...
def get_args_parser():
    ...
    parser.add_argument('--test_data_path', default='', type=str,
                        help='external test dataset root directory')
    parser.add_argument('--test_label_path', default='', type=str,
                        help='path to the external test label CSV')
...
def main(args):
    ...
    dataset_train, dataset_val, dataset_test, label_map = create_external_datasets(args)
