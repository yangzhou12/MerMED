...
# --------------------------------------------------------
"""Finetune the MerMED-FM ViT backbone for multi-label classification.

Same as ``main_finetune.py`` but uses a multi-label dataset loader and a
``BCEWithLogitsLoss`` objective, reporting macro/micro AUC, F1 and per-label metrics.
"""
...
from util.datasets_multilabel import build_dataset
from util.pos_embed import interpolate_pos_embed
import util.misc as misc
from util.misc import NativeScalerWithGradNormCount as NativeScaler

from engine_finetune_multilabel import train_one_epoch, evaluate, multilabel_evaluate
import models_vit
...
def get_args_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--modality', type=str, default=None, choices=['cfp', 'oct', 'cxr', 'pathology', 'CT', 'US', 'skin'],
    ...
    # Add new argument for classification type
    parser.add_argument('--classification_type', type=str, default='multi-class', choices=['multi-class', 'multi-label'],
                        help='Type of classification task (multi-class or multi-label)')
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.distributed:  # Only use DistributedSampler when actually running distributed
    ...
    def load_checkpoint_model(checkpoint):
        ...
        if 'model' in checkpoint:
            ...
            prefix_to_remove = "module.backbone."
        else:
        ...
        if prefix_to_remove:
            ...
            for old_key, value in checkpoint_model.items():
                # Only remove prefix if it's at the start of the key
                new_key = old_key[len(prefix_to_remove):] if old_key.startswith(prefix_to_remove) else old_key
    ...
    if args.finetune and not args.eval:
        ...
        checkpoint_model = load_checkpoint_model(checkpoint)

        # Get current model state for shape comparison
        state_dict = model.state_dict()
        
        # Define keys to check and potentially remove
        keys_to_remove = [
    ...
    # Modify criterion selection based on classification type
    if args.classification_type == 'multi-class':
    ...
    def do_evaluation(data_loader, mode, epoch, evaluate_fairness_this_epoch=False):
        if args.classification_type == 'multi-label':
            return multilabel_evaluate(
                data_loader, model, device, output_dir, epoch, mode, args.nb_classes,
            ...
            # For multi-class, use the evaluate function (fairness support would need to be added separately)
            # Note: For full fairness support in multi-class, consider using engine_finetune_fairness
            return evaluate(
                data_loader, model, device, output_dir, epoch, mode, args.nb_classes,
    ...
    if args.eval:
        ...
        if args.classification_type == 'multi-label':
            test_stats, auc_roc = multilabel_evaluate(
                data_loader_test, model, device, output_dir, epoch=0, mode='test', num_classes=args.nb_classes,
                evaluate_fairness=args.evaluate_fairness,
                sensitive_attr_names=args.sensitive_attr
            )
        else:
            # For multi-class, use the evaluate function
            # Note: For full fairness support in multi-class, consider using engine_finetune_fairness
            test_stats, auc_roc = evaluate(
                data_loader_test, model, device, output_dir, epoch=0, mode='test', num_class=args.nb_classes,
                task_type='multi-class'
            )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_stats, val_auc_roc = do_evaluation(data_loader_val, 'val', epoch, evaluate_fairness_this_epoch)

        if val_auc_roc > max_auc:
            max_auc = val_auc_roc
            if args.output_dir:
                misc.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp,
        ...
        if epoch == (args.epochs - 1):
            ...
            test_stats, test_auc_roc = do_evaluation(data_loader_test, 'test', epoch, args.evaluate_fairness)
