...
def train_mermed(args):
    ...
    # ============ preparing data ... ============
    transform = DataAugmentationMerMED(
        args.global_crops_scale,
        args.local_crops_scale,
        args.local_crops_number,
    )
    dataset = MerMEDDataset(args.data_path, transform=transform)
    ...
    params_groups = utils.my_get_params_groups(
        student.module, weight_decay=args.weight_decay, layer_decay=args.layer_decay)
    ...
    momentum_schedule = utils.cosine_scheduler(
        ...
        # warmup_epochs=10, start_warmup_value=1
    )
    ...
    print("Starting MerMED training !", flush=True)
    for epoch in range(start_epoch, args.epochs):
        data_loader.batch_sampler.set_epoch(epoch)

        # ============ training one epoch of MerMED ... ============
        train_one_epoch(
            student,
            teacher,
            teacher_without_ddp,
...
def train_one_epoch(
    ...
    for i, (images, labels) in enumerate(data_loader):
        ...
        with torch.amp.autocast('cuda', enabled=fp16_scaler is not None, dtype=torch.float16):
            ...
            student_output = memory_bank(student_output, labels)
            teacher_output = memory_bank(teacher_output, labels, update=True)
...
class DataAugmentationMerMED(object):
    def __init__(self, global_crops_scale, local_crops_scale, local_crops_number):
        self.normalizations = {
            ...
            'pathology': transforms.Normalize((0.72139378, 0.60500935, 0.70543986), (0.24855434, 0.27136655, 0.24570293)),
            'CT': transforms.Normalize((0.33009237,), (0.32522697,)),
            'US': transforms.Normalize((0.14694033,), (0.1806314,)),
            'skin': transforms.Normalize((0.66138601, 0.51889024, 0.47144768), (0.23963617, 0.21994001, 0.22855408))
        }

        # ------------------------------------------------------------------------------
        # 1. Shared Flip & Color Jitter
        # ------------------------------------------------------------------------------
        # Removed RandomGrayscale entirely, as color is usually crucial for skin images.
        ...
        # 2. First Global Crop
        # ------------------------------------------------------------------------------
        # Removed RandomPerspective and toned down rotation to ±30°
        # for less extreme geometric distortion.
        self.global_transfo1 = transforms.Compose(
            [
                ...
                # GaussianBlur can still be used for mild blur augmentation
                transforms.RandomApply(
                    [utils.GaussianBlur([0.1, 1.0])],
                    p=1.0  # always applied in global_transfo1
                ),
                # transforms.Normalize(mean=..., std=...) if you need normalization
            ]
        )

        # ------------------------------------------------------------------------------
        # 3. Second Global Crop
        # ------------------------------------------------------------------------------
        # Same structure but with slight differences in probabilities.
        # Removed Solarize and Perspective for clarity.
        self.global_transfo2 = transforms.Compose(
            [
                ...
                # Reduced probability of GaussianBlur
                transforms.RandomApply(
                    [utils.GaussianBlur([0.1, 1.0])],
                    p=0.1
                ),
        ...
        # 4. Local Crops
        # ------------------------------------------------------------------------------
        # Typically smaller crops (e.g., 96×96). Removed Perspective for local as well.
        ...
        self.local_transfo = transforms.Compose(
            [
                ...
                # Mild blur with moderate probability
                transforms.RandomApply(
                    [utils.GaussianBlur([0.1, 1.0])],
                    p=0.5
                ),
    ...
    def __call__(self, image, modality):
        normalize = transforms.Compose(
            [
                transforms.ToTensor(),
                self.normalizations[modality]
            ]
        )

        crops = []
        crops.append(normalize(self.global_transfo1(image)))
        crops.append(normalize(self.global_transfo2(image)))
        for _ in range(self.local_crops_number):
            crops.append(normalize(self.local_transfo(image)))
        return crops

if __name__ == "__main__":
    parser = argparse.ArgumentParser("MerMED", parents=[get_args_parser()])
